        ...
        <v-row class="d-flex align-center justify-center">
            <div class="grid-container">
                <div v-for="(row, rowIndex) in grid" :key="rowIndex" class="row">
                    <div v-for="(col, colIndex) in row" :key="colIndex" class="square" :style="{ backgroundColor: col.color }" @mouseover="toggleColor(rowIndex, colIndex)" @mouseleave="toggleColor(rowIndex, colIndex)"></div>
                </div>
            </div>
        </v-row>
    </div>
